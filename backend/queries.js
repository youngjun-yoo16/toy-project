...
const getHobbiesWithUserName = (_request, response) => {
    pool.query('SELECT hobbies.id, fk_user_id, name, sport, instrument FROM users INNER JOIN hobbies on hobbies.fk_user_id = users.id', (error, results) => {
        if (error) {
            throw error
        }
        response.status(200).json(results.rows)
...
const getHobbiesByUserId = (request, response) => {
    const id = parseInt(request.params.id)
    pool.query('SELECT hobbies.id, fk_user_id, name, sport, instrument FROM users INNER JOIN hobbies ON hobbies.fk_user_id = users.id WHERE users.id = $1', [id], (error, results) => {
        if (error) {
            throw error
        }
        response.status(200).json(results.rows)
...
const createHobby = (request, response) => {
    ...
    pool.query('INSERT INTO hobbies (fk_user_id, sport, instrument) VALUES ($1, $2, $3)', [fk_user_id, sport, instrument], (error, results) => {
        if (error) {
            throw error
        }
